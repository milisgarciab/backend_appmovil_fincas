...
export class AuthService {
  ...
  async register(input: RegisterInput): Promise<UsuarioPublico> {
    if (!input.nombre_usuario || !input.correo_electronico || !input.contrasena) {
      throw new AppError('nombre_usuario, correo_electronico y contrasena son obligatorios', 400, 'VALIDATION_ERROR');
    }
  ...
  async login(input: LoginInput) {
    ...
    if (!usuario) throw new AppError('Correo o contraseña incorrectos', 401, 'INVALID_CREDENTIALS');
    const contrasenaValida = await bcrypt.compare(input.contrasena, usuario.contrasena);
    if (!contrasenaValida) throw new AppError('Correo o contraseña incorrectos', 401, 'INVALID_CREDENTIALS');
    const payload = { id: usuario.id, rol_id: usuario.rol_id };
    const accessToken = generateAccessToken(payload);
    const refreshToken = generateRefreshToken(payload);
    return { accessToken, refreshToken, expiresIn: ACCESS_TOKEN_EXPIRES_IN_SECONDS, user: this.toPublicUser(usuario) };
  }

  async refresh(refreshToken?: string) {
    if (!refreshToken) throw new AppError('refreshToken es obligatorio', 400, 'VALIDATION_ERROR');
    let payload;
    try { payload = verifyRefreshToken(refreshToken); }
    catch { throw new AppError('Refresh token inválido o expirado', 401, 'INVALID_REFRESH_TOKEN'); }
    const usuario = await this.usuarioRepository.findById(payload.id);
    if (!usuario) throw new AppError('Refresh token inválido o expirado', 401, 'INVALID_REFRESH_TOKEN');
  ...
  private toPublicUser(u: { id: number; nombre_usuario: string; correo_electronico: string; telefono: string | null; rol_id: number }): UsuarioPublico {
    return { id: u.id, nombre_usuario: u.nombre_usuario, correo_electronico: u.correo_electronico, telefono: u.telefono, rol_id: u.rol_id };
  }
}
